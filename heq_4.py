...
while True:
    try:
        coef_num = int(input('Enter number of coefficients for the FIR filter (enter 0 for best - computed automatically): '))
        if coef_num >= 0:
            break
        else:
...
for i in range(hpfilesnum):
    ...
    for j in range(targetsnum):
        ...
        plt.xlim([20, 20000])
        plt.ylim([-80, 40])
        plt.title(f'Desired Filter Response vs Actual Filter Response (n = {len(filter_coefs)})')
